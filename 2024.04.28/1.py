...
def pick_resistors(nominal: int) -> dict[str, tuple] | None:
	""" Принимает номинал резистора и возвращает словарь с наиболее близкими значениями из стандартных """
	...
	for key in nominals.keys():
		result_dict[key] = tuple(map(lambda x: abs(nominal - x), nominals[key]))
		result_dict[key] = tuple(filter(lambda x: min(result_dict[key]) == abs(x - nominal), nominals[key]))
	return(result_dict)
